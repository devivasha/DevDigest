...
- **Q1 — Attached-specs selection.** There is no per-PR "attached specs" set; project-context docs are attached per **agent** (`AgentRow.attachedDocPaths`). → **default:** the Brief gathers the **union of the workspace's agents' `attachedDocPaths`** for the PR's repo, deduped and capped (≤5 docs, total byte budget ~12KB), read best-effort via the existing `resolveSpecPaths` / `readDocument` review-run helpers. Confirm this selection (alternative: no specs in v1, rely on intent/blast/issue only).
...
- **Rec2 — Endpoint vs file-path grounding.** `Risk.file_refs` may hold **endpoints** (e.g. `GET /pulls/:id`), which cannot be fs-verified against a clone. Recommendation: path-ground only **file-path-shaped** refs (no whitespace, contains `/` or a file extension, not `METHOD /route`) via `clonePathFor` + fs-stat (the onboarding `isGroundedPath` precedent); endpoint-shaped refs pass through as **text** (the client renders them non-navigating). This satisfies AC-8 ("no fabricated path rendered as a clickable link") without dropping legitimate endpoint mentions. Flagged so it is a deliberate, reviewed boundary.
...
- **T4**
  ...
  - **Acceptance:** File exists and renders via `renderPrompt` without unresolved placeholders; the instruction text names all five reused input categories and the content bounds; T5/T9 exercise it end-to-end. (→ AC-1, AC-4, AC-18, AC-21)
  ...
  - **Action:** Create `server/src/modules/brief/service.ts` exporting `class BriefService { constructor(private container: Container, private logger?: Logger) }` with a private `repo = new ReviewRepository(container.db)`.
    ...
    - private `compute(workspaceId, prId)`:
      ...
      2. **Gather reused inputs (each best-effort, try/catch → null/empty; AC-18):**
         ...
         - `specTexts` — union of the workspace agents' `attachedDocPaths` (Q1 default), read best-effort via `readDocument` (`../project-context/documents.js`), capped (≤5 docs, ~12KB budget).
      3. **Untrusted wrapping (AC-21):** wrap PR title/body, linked-issue body, and each spec text with `wrapUntrusted('pr'|'issue'|'spec:<src>', text)` from `../../platform/prompt.js` before assembling the user message. Deterministic pieces (blast summary, group stats) need no wrapping.
      ...
      5. **Path-ground (AC-8, Rec2):** `clonePath = this.container.git.clonePathFor(repoRef)`; for each `review_focus` item and each `risk.file_refs` entry that is **file-path-shaped**, keep only paths that resolve to a real in-tree file (onboarding `isGroundedPath` pattern: `resolve(clonePath, p)` must stay under `resolve(clonePath)+sep` and `stat().isFile()`); drop unverifiable path-shaped refs. Endpoint-shaped refs pass through. If the clone is absent, all path-shaped refs drop (best-effort).
...
- **Fabricated / out-of-tree file paths rendered as links.** → clone-fs path-existence grounding drops path-shaped refs that don't resolve to an in-tree file (Rec2); the client further degrades to a non-navigating control when no URL can be built (AC-14).
