...
### GitHub Actions (per-PR) — the shipped workflow

This is wired up and lives at [`.github/workflows/evals.yml`](../.github/workflows/evals.yml). It runs
on a PR **only** when a harness artifact changes, and runs **only** the evals that change affects —
driven by [`scripts/ci-detect.mjs`](scripts/ci-detect.mjs), which maps changed files onto suites:
...
*substring*, so a bare `architecture-reviewer` would also run `-ab` and `-lite`. One job per artifact
means a failure names the exact artifact, and an empty detect list (`[]`) skips the job entirely.

**To turn it on / tune it:**
- Add the OpenRouter key as a repo **Actions secret** `OPENROUTER_API_KEY` (Settings → Secrets and
  variables → Actions). The proxy container reads it straight from the job `env`.
- Mark `skills` / `agents` as **required checks** in branch protection to actually gate merges.
- The `workflow` job is `continue-on-error: true` (advisory) because tool-tier activation/dispatch is
  flaky on cheap models under load (caveats above). Flip that one line to make it blocking.
